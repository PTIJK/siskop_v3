...
export function selectionEnabled() {
  return tenantDomainsEnabled() && process.env.TENANT_LOGIN_SELECTION_ENABLED === 'true'
}
export function switchingEnabled() {
  return selectionEnabled() && process.env.TENANT_SWITCHING_ENABLED === 'true'
}
...
export function tenantOrigin(slug: string) {
  return new URL(tenantLoginUrl(slug)).origin
}
