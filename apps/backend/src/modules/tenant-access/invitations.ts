...
export async function inviteMembership(tenantId: string, membershipId: string): Promise<MembershipInvitationResponse> {
  ...
  await db.$transaction(async (tx) => {
    await tx.membershipInvitation.updateMany({
      where: { membershipId, acceptedAt: null },
      data: { expiresAt: new Date() }
    })
    await tx.membershipInvitation.create({
      data: { membershipId, tokenHash: digest(token), email: user.email.toLowerCase(), expiresAt }
    })
  })
...
export async function acceptInvitation(token: string, idToken: string) {
  ...
  const invitation = await db.membershipInvitation.findUnique({
    where: { tokenHash: digest(token) },
    include: { membership: { include: { tenant: true } } }
  })
  if (!invitation || invitation.acceptedAt || invitation.expiresAt <= new Date())
    throw forbidden('Undangan tidak valid atau kedaluwarsa.')
  if (
    invitation.email !== proof.email ||
    invitation.membership.email.toLowerCase() !== proof.email ||
    !invitation.membership.tenant.isActive ||
    invitation.membership.identityId ||
    invitation.membership.isPlatformAdmin
  )
    throw forbidden('Undangan bukan untuk akun ini.')
  await db
    .$transaction(async (tx) => {
      const identity = await tx.accountIdentity.upsert({
        where: { firebaseUid: proof.uid },
        update: {},
        create: { firebaseUid: proof.uid }
      })
      if (!identity.isActive) throw forbidden('Akun tidak aktif.')
      const existing = await tx.user.findUnique({
        where: { identityId_tenantId: { identityId: identity.id, tenantId: invitation.membership.tenantId } }
      })
      if (existing) throw conflict('Akun ini sudah memiliki keanggotaan di koperasi tersebut.')
      const used = await tx.membershipInvitation.updateMany({
      ...
      const linked = await tx.user.updateMany({
        where: {
          ...
          tenantId: invitation.membership.tenantId,
          identityId: null,
          email: invitation.membership.email,
          isPlatformAdmin: false,
          tenant: { isActive: true }
        },
        data: { identityId: identity.id, isActive: true, passwordHash: null, authProvider: proof.provider }
      })
      if (linked.count !== 1) throw conflict('Keanggotaan berubah. Minta undangan baru.')
    })
    .catch((err: unknown) => {
      if (typeof err === 'object' && err !== null && 'code' in err && err.code === 'P2002')
        throw conflict('Akun ini sudah memiliki keanggotaan di koperasi tersebut.')
      throw err
    })
