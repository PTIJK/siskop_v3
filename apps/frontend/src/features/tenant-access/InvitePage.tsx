...
import {
  ...
  firebaseError,
  type AuthMethod
} from '@/features/onboarding/firebase'
...
export default function InvitePage() {
  ...
  async function accept(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setDone(true)
      setMessage(`Akses ke ${result.tenantName} berhasil ditambahkan.`)
    } catch (err) {
      setMessage(firebaseError(err) ?? (err instanceof Error ? err.message : 'Undangan belum dapat diterima.'))
    } finally {
  ...
  async function verify(form: HTMLFormElement | null, register: boolean) {
    ...
    try {
      ...
      setMessage(
        'Periksa email verifikasi Anda. Setelah terverifikasi, kembali ke tautan ini dan pilih Terima undangan.'
      )
    } catch (err) {
      setMessage(firebaseError(err) ?? 'Belum dapat mengirim verifikasi.')
    } finally {
      await clearFirebaseIdentity()
      setPending(false)
    }
    ...
    <main className='mx-auto flex min-h-screen max-w-md flex-col justify-center gap-5 p-6'>
      ...
      {done ? (
        <>
          ...
          <a className='underline' href={centralLocation('/login')}>
            Masuk ke koperasi
          </a>
        </>
      ) : (
        <form onSubmit={(e) => void accept(e)} className='flex flex-col gap-4'>
          ...
          {method === 'password' ? (
            <>
              ...
              />
              <Label htmlFor='invite-password'>Kata sandi</Label>
          ...
          {method === 'password' ? (
            <div className='flex flex-col gap-2'>
              <Button
                ...
                variant='outline'
                disabled={pending}
                onClick={(e) => void verify(e.currentTarget.form, true)}
              >
              ...
              <Button
                ...
                variant='ghost'
                disabled={pending}
                onClick={(e) => void verify(e.currentTarget.form, false)}
              >
          ...
          {!token.current ? (
            <p role='alert'>Tautan undangan tidak lengkap. Minta tautan baru kepada pengelola.</p>
          ) : null}
        </form>
      )}
    </main>
