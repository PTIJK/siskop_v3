...
export default function HandoffPage({ mode }: { mode: "start" | "authorize" | "callback" | "logout" }) {
  ...
  useEffect(() => {
    if (run.current) return; run.current = true;
    const values = new URLSearchParams(window.location.hash.slice(1) || window.location.search);
    const attempt = values.get("attempt"), state = values.get("state"), code = values.get("code");
    window.history.replaceState(null, "", window.location.pathname);
    void (async () => {
      if (mode === "start") {
        const data = await accessApi<{ authorizeUrl: string }>("/start", { attempt });
        if (new URL(data.authorizeUrl).origin !== new URL(centralLocation("/")).origin) throw new Error("Alamat masuk tidak valid.");
        ...
        useAuth.getState().setSession(session); navigate("/dashboard", { replace: true });
      } else {
        await accessApi("/logout", {}); useAuth.getState().clear(); navigate("/login", { replace: true });
      }
    })().catch(e => setError(e instanceof Error ? e.message : "Belum dapat masuk."));
  }, [mode, navigate]);
  return <main className="mx-auto flex min-h-screen max-w-md flex-col justify-center gap-4 p-6"><h1 className="text-xl font-semibold">{mode === "logout" ? "Keluar" : "Membuka koperasi"}</h1>{error ? <><p role="alert">{error}</p><a className="underline" href={centralLocation("/login?select=1")}>Kembali ke halaman masuk</a></> : <p role="status">Mohon tunggu…</p>}</main>;
}
