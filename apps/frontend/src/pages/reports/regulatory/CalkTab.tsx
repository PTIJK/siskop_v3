...
function NarrativeEditor({
  ...
  const save = async () => {
    setIsSaving(true);
    try {
      const result = await apiPut<{ updatedAt: string }>("/reports/regulatory/calk/narrative", { section, content: text });
      onSaved(section, text, result.updatedAt);
      ...
      <CardContent className="space-y-2">
        {canEdit ? (
          ...
          <>
            ...
            <p className="text-xs text-muted-foreground">{updatedAt ? `Terakhir diperbarui ${formatTanggalIndonesia(updatedAt)}` : ""}</p>
          </>
        )}
      </CardContent>
    </Card>
...
export function CalkTab() {
  ...
  const fetchReport = async () => {
    setIsLoading(true);
    try {
      const result = await apiFetch<Calk>(`/reports/regulatory/calk?from=${from}&to=${to}`);
      setData(result);
    } catch (err) {
      const message = err instanceof ApiRequestError ? err.message : "Terjadi kesalahan";
      toast({ title: "Gagal memuat CALK", description: message, variant: "destructive" });
    } finally {
      setIsLoading(false);
    }
  };
