...
export function SavingDetailPage() {
  ...
  const handleTransaction = async () => {
    ...
    try {
      ...
      await apiPost(`/savings/${id}/${endpoint}`, { amount: parseFloat(amount), note: note || undefined });
      toast({ title: action === "deposit" ? "Setoran berhasil" : "Penarikan berhasil" });
      setAction(null);
      setAmount("");
      setNote("");
      await queryClient.invalidateQueries({ queryKey: ["savings", id] });
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      <PageHeader title={saving.savingConfig.name} breadcrumb={[{ label: "Simpanan", href: "/savings" }, { label: saving.savingConfig.name }]} />
      ...
      <Dialog open={!!action} onOpenChange={(o) => !o && setAction(null)}>
        <DialogContent className="sm:max-w-sm">
          <DialogHeader>
            <DialogTitle>{action === "deposit" ? "Setor Dana" : "Tarik Dana"}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            ...
            <div className="space-y-1.5">
              <Label>Nominal</Label>
              <Input type="number" placeholder="0" min="1" value={amount} onChange={(e) => setAmount(e.target.value)} />
            </div>
            <div className="space-y-1.5">
              <Label>Catatan (opsional)</Label>
              <Textarea rows={2} placeholder="Keterangan transaksi" value={note} onChange={(e) => setNote(e.target.value)} />
            </div>
          </div>
          <DialogFooter>
